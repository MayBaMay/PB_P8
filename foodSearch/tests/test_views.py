...
class UserActivityTestCase(TransactionTestCase):
    ...
    def test_save_favorite(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(Favorite.objects.filter(user=self.user2, substitute=self.product1).exists(), True)
        self.client.logout()
    ...
    def test_unsave_favorite(self):
        ...
        response = self.client.post(reverse('foodSearch:load_favorite'),
                                    ...
                                    HTTP_X_REQUESTED_WITH='XMLHttpRequest')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(Favorite.objects.filter(user=self.user1, substitute=self.product1).exists(), False)
        self.client.logout()
