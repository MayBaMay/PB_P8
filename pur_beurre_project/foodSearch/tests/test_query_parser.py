...
class FilterFoundProductsTestCase(TestCase):
    ...
    def setUp(self):
        Product.objects.create(name="Fàke product for db", brands="brand fake", reference="1")
        Product.objects.create(name="Second fake prôduct", brands="the wrong one", reference="2")
    ...
    #     parser = QueryParser(self.query_name_brands)
    #     self.assertEqual(parser.occurences()['1'], 4)
    #     self.assertEqual(parser.occurences()['2'], 2)
    #     self.assertEqual(parser.occurences()['3'], 1)
    #     parser = QueryParser(self.one_result_query)
